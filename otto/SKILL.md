---
name: otto
description: Execute a .plans/<slug> slice plan as a dependency-ordered wave loop using Claude Code subagents (no tmux) — parallel slices run in isolated git worktrees, each lands as a commit you review afterward, and learnings compound across waves. Use when the user wants to run a plan, execute or land slices, check slice progress, or mentions otto, slices, or a .plans folder.
...
- **`index.ts`** — the deterministic git + state CLI, and the *only* writer of `state.json` and
  ...
  flags) so you invoke it as a path — no flags to type. Set `OTTO` to `index.ts` in *this skill's
  own directory* (the "Base directory for this skill" path from the invocation):

  ```
  OTTO="<this-skill-dir>/index.ts"      # then:  $OTTO <cmd> …
  ```
...
- **`workflow/wave.js`** — the single-wave fan-out (in this skill's directory), run via the
  `Workflow` tool: one subagent per slice, each in its otto-made worktree.

## Commands

| Invocation | What it does |
|---|---|
| `/otto run <slug> [-s range] [-m worktree\|inline]` | Run the wave loop (the main thing). |
| `/otto status <slug>` | Show slice statuses. |
...
`/otto run` is a loop: compute the next wave from `state.json`, set up worktrees, fan out one
...
Each landed slice is a commit on your branch (otto never pushes), so the loop runs unattended and you
