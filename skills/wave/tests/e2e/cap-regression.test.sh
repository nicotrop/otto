...
wave done deep-chain 01-a >/dev/null
wave done deep-chain 02-b >/dev/null
wave done deep-chain 03-c >/dev/null
wave done deep-chain 04-d >/dev/null
...
check "remaining depth (status) is 1" "remaining depth: 1  (4/5 slices done)" \
  "$(wave status deep-chain 2>/dev/null | grep -o 'remaining depth:.*')"
...
check "wave 5: 05-e actually runs" "05-e" \
  "$(wave wave deep-chain -w 5 2>/dev/null | node -e "const w=JSON.parse(require('fs').readFileSync(0));process.stdout.write(w.slices.map(s=>s.key).join(','))")"
