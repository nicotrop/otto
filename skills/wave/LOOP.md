# The `/wave run` wave loop

How to drive `/wave run <slug> [-s range] [-m mode]`. Carry the user's `-s`/`-m` flags (defaults:
all slices, `worktree`) through **every** `start`/`init` call so they agree.

Let `WAVE="<this-skill-dir>/index.ts"` (the skill's own directory — the "Base directory for this
skill" path from the invocation).

**Step 0 — once, before anything else:** run `$WAVE validate <slug>`. On any error it prints the
`error:` lines to stderr, exits non-zero, and stops the run — an invalid, hand-edited plan can't start.
Warnings (e.g. an orphan slice `.md`) print but don't block. wave does **not** commit `.plans/`; it
...
1. **Compute AND set up the next wave — one command.** Run `$WAVE wave <slug> -w <N> [-s range] [-m mode] [-b buffer]`,
   where `<N>` is the current wave count (1 on the first pass, +1 each loop). `-b` sets how many extra
   waves past the graph's critical-path depth wave tolerates before halting (default 2).
   ...
   - `cap_reached: true` → the wave count passed `wave_cap` (≈ critical-path depth + buffer), so the loop is
     spinning without landing slices. **Stop the loop.** Report that wave hit the iteration cap, run
     `$WAVE status <slug>` to list what's still `pending`, and tell the user the loop isn't making
...
3. **Land each slice, in order.** For each `key` in `ran`, **first check its result**:
   ...
   - Otherwise run `$WAVE land <slug> <key> -L "<that slice's learnings from the result>"`. This rebases
   ...
   **uncommitted** (wave never commits `.plans/`). Committing the code is what lets the next wave
   compound on real code; the next wave's worktrees are cut from this new HEAD and re-seeded with the
   updated `learnings.md`.
   - **On CONFLICT** (wave exits non-zero with a conflict message): **STOP the loop.** A same-wave
...
- **Single writer.** Only wave writes `state.json` / `learnings.md`, only in the main tree.
  Subagents *return* learnings (in their result JSON); the skill passes them to `land -L`. Never
  hand-edit `state.json` mid-run.
- **wave never commits `.plans/`.** Specs, `state.json`, and `learnings.md` stay as dirty
  working-tree files for the run; wave copies them into each worktree at wave time and excludes them
  there so an agent can't commit them back. The user commits `.plans/` themselves if they want it.
- **Base ref is always current HEAD.** wave handles this; never point worktrees at develop/main.
- **Every landed slice's code is committed.** That is what makes the next wave compound. The loop
  runs unattended; you review the commit stack afterward (`git log`), and can squash/amend since wave
  never pushes.
...
Each landed slice is a commit on your branch (wave never pushes). Review with `git log` / `git diff`,
QA in preview, and squash/amend/reset as you like. To review in batches, scope the run with `-s`
(e.g. `/wave run <slug> -s 1-4`) so it stops after slice 4 — then inspect those commits before
running the next range.

## Resuming

If a run is interrupted, just `/wave run <slug>` again. Unfinished slices are still `pending` (their
...
with a live worktree (work was in flight). Use `/wave reset <slug> <slice>` to scrap a stuck slice and
start it over.
...
the wave counter restarts at 1). To tighten the backstop on a resume, run `$WAVE status <slug>` first:
it prints `remaining depth: <n>`, the depth of the still-unfinished subgraph. Pass that as `-d <n>`
on the `wave` calls (`$WAVE wave <slug> -w <N> -d <n> …`) so wave halts a stuck resume sooner.
